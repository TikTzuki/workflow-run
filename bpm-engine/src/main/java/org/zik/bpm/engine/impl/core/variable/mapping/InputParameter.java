...
public class InputParameter extends IoParameter
{
    ...
    public InputParameter(final String name, final ParameterValueProvider valueProvider) {
    ...
    protected void execute(final AbstractVariableScope innerScope, final AbstractVariableScope outerScope) {
        final Object value = this.valueProvider.getValue(outerScope);
        InputParameter.LOG.debugMappingValueFromOuterScopeToInnerScope(value, outerScope, this.name, innerScope);
