...
{
    public CaseActivityInstanceUpdateListener(final CmmnHistoryEventProducer historyEventProducer) {
    ...
    protected HistoryEvent createHistoryEvent(final DelegateCaseExecution caseExecution) {
        ...
        if (this.historyLevel.isHistoryEventProduced(HistoryEventTypes.CASE_ACTIVITY_INSTANCE_UPDATE, caseExecution)) {
            return this.eventProducer.createCaseActivityInstanceUpdateEvt(caseExecution);
        }
        return null;
    }
}
