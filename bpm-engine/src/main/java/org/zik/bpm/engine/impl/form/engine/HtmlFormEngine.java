...
public class HtmlFormEngine implements FormEngine
{
    ...
    protected void renderDatePicker(final FormField formField, final HtmlDocumentBuilder documentBuilder) {
        ...
        final String dateFormat = (String)formField.getType().getInformation("datePattern");
        if (!isReadOnly) {
            inputField.attribute("uib-datepicker-popup", dateFormat).attribute("is-open", String.format("dateFieldOpened%s", formFieldId));
        }
    ...
    protected void renderInvalidDateMessage(final FormField formField, final HtmlDocumentBuilder documentBuilder) {
        ...
        secondDivElement.attribute("ng-show", secondExpression).attribute("class", "help-block").textContent("Invalid date format: the date should have the pattern '" + formField.getType().getInformation("datePattern") + "'");
    ...
    protected void addCommonFormFieldAttributes(final FormField formField, final HtmlElementWriter formControl) {
        ...
        final String formFieldId = formField.getId();
        formControl.attribute("class", "form-control").attribute("name", formFieldId);
        if (!formField.isBusinessKey()) {
            formControl.attribute("cam-variable-type", typeName).attribute("cam-variable-name", formFieldId);
        }
        else {
            formControl.attribute("cam-business-key", null);
        }
